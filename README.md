...
## Notes
### Design Evolution and Decision
Prior attemps of using stereo matching to gain depth image. Testing reveal that the featureless image generated by Gazebo
lacked the distinct high-frequency textures required for block-matching algorithms to calculate accurate pixel correspondences. This resulted in severe holes and noise in the depth image.

**The Solution:** We modified the camera in the Gazebo simulation environment from rgb to rgbd, expose the raw depth, essentially turn the setup from three RGB cameras to one RGB-D camera. This architectural pivot completely eliminated the texturing problem, and provided clean, high-fidelity spatial data directly to FoundationPose.


### Current Grey Frame Issue
