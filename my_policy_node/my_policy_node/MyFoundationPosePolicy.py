...
class MyFoundationPosePolicy(Policy):
    ...
    # def get_card_pose_in_base_frame(self, T_camera_card: np.ndarray) -> Pose:
    #     """
    #     Transforms the NIC card pose from the Camera Frame into the Robot Base Link Frame,
    #     and directly prints the static delta matrix relative to self.port_transform.
        
    #     Input:  T_camera_card -> (4,4) numpy array tracking matrix from FoundationPose
    #     Output: Pose           -> ROS 2 Geometry Pose relative to 'base_link'
    #     """
    #     camera_frame = "center_camera/optical" 
    #     base_frame = "base_link"
    ...
    #         )
    #     except TransformException as ex:
    #         self.get_logger().error(f"Could not look up camera frame '{camera_frame}': {ex}")
    #         return None
    ...
    #     # 3. Compute the Vision Matrix Chain (Object relative to Robot Base)
    #     T_base_card = T_base_camera @ T_camera_card

    #     # =========================================================================
    #     # --- DIRECT DELTA LOGGING BLOCK ---
    #     # =========================================================================
    #     if hasattr(self, 'port_transform') and self.port_transform is not None:
    #         try:
    #             # Build the 4x4 Ground Truth matrix directly from the stored attribute (T_base_port)
    #             T_base_port = np.eye(4)
    #             T_base_port[0, 3] = self.port_transform.translation.x
    #             T_base_port[1, 3] = self.port_transform.translation.y
    #             T_base_port[2, 3] = self.port_transform.translation.z
                
    #             q_port = [
    #                 self.port_transform.rotation.x, 
    #                 self.port_transform.rotation.y, 
    #                 self.port_transform.rotation.z, 
    #                 self.port_transform.rotation.w
    #             ]
    #             T_base_port[:3, :3] = R.from_quat(q_port).as_matrix()
                
    #             # Compute the static structural delta transformation:
    #             # T_card_port = (T_base_card)^-1 * T_base_port
    #             T_card_port = np.linalg.inv(T_base_card) @ T_base_port
                
    #             # Extract static translation vectors and rotation quaternions
    #             trans_offset = T_card_port[:3, 3]
    #             quat_offset = R.from_matrix(T_card_port[:3, :3]).as_quat()
                
    #             self.get_logger().info(
    #                 f"\n[STATIC CAD OFFSET FOUND] "
    #                 f"Translation (X, Y, Z): [{trans_offset[0]:.5f}, {trans_offset[1]:.5f}, {trans_offset[2]:.5f}] | "
    #                 f"Quaternion (X, Y, Z, W): [{quat_offset[0]:.5f}, {quat_offset[1]:.5f}, {quat_offset[2]:.5f}, {quat_offset[3]:.5f}]"
    #             )
    #         except Exception as log_ex:
    #             self.get_logger().warn(f"Failed to process delta math printout: {log_ex}")
    #     else:
    #         self.get_logger().warn("self.port_transform has not been populated by insert_cable yet.")
    #     # =========================================================================

    #     # 4. Pack the resulting 4x4 matrix into a clean ROS 2 Pose Message
    #     card_pose = Pose()
    #     card_pose.position.x = float(T_base_card[0, 3])
    #     card_pose.position.y = float(T_base_card[1, 3])
    #     card_pose.position.z = float(T_base_card[2, 3])
        
    #     q_card_output = R.from_matrix(T_base_card[:3, :3]).as_quat()
    #     card_pose.orientation.x = float(q_card_output[0])
    #     card_pose.orientation.y = float(q_card_output[1])
    #     card_pose.orientation.z = float(q_card_output[2])
    #     card_pose.orientation.w = float(q_card_output[3])
