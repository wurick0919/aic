...
class AicModel(LifecycleNode):
    def __init__(self):
        ...
        #     reliability=ReliabilityPolicy.BEST_EFFORT,     # Crucial: Don't block if packets drop!
        #     history=HistoryPolicy.KEEP_LAST,               # Only care about the latest data
        #     depth=1,                                       # Strictly hold exactly 1 frame max
        #     durability=DurabilityPolicy.VOLATILE           # Do not try to catch up on old missed messages
        # )
        
        # self.observation_sub = self.create_subscription(
        #     Observation, "observations", self.observation_callback, qos_profile=image_qos 
        # )
        # self.center_depth_sub = self.create_subscription(
        #     Image, "/center_camera/depth_image/compressed", self.center_depth_callback, qos_profile=image_qos 
        # )
        self.observation_sub = self.create_subscription(
    ...
    def on_shutdown(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.destroy_subscription(self.center_depth_sub_sub)
        self.observation_sub = None
        self.center_depth_sub_sub = None
    ...
    def observation_callable(self):
        return self._observation_msg, self._center_depth_msg
    
    # def depth_callable(self):
    #     return self._center_depth_msg
    ...
    def action_thread_func(self, goal_handle: ServerGoalHandle):
        self._action_thread_result = self._policy.insert_cable(
            ...
            # get_depth = lambda: self.depth_callable(),
